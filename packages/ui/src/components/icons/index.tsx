/* ================
  ...
  names of icons are one word without capital letters or hyphens.
  ONLY if you add a duplicate icon in the filled version add same name with -fill.
*/
...
export const SvgWrapper = styled.div<{ size: number, clickable: boolean }>`
  display: inline-block;
  flex: 0 0 ${({ size }): string => (size ? `${size}px` : '24px')};
  width: ${({ size }): string => (size ? `${size}px` : '24px')};
  height: ${({ size }): string => (size ? `${size}px` : '24px')};
  min-width: ${({ size }): string => (size ? `${size}px` : '24px')};
  min-height: ${({ size }): string => (size ? `${size}px` : '24px')};
  position: relative;
  color: inherit;
  cursor: ${({ clickable }) => clickable ? 'pointer' : 'auto'};
`;

export const InlineSvg = styled.svg<{ color: string }>`
  position: absolute;
  top: 0;
  right: 0;
  bottom: 0;
...
/* =====================================================
    WHEN CREATING A NEW ICON, ADD THE ICON TO THIS LIST
  ======================================================
*/
export const glyphs = ['alerttriangle', 'archive', 'arrowback', 'arrowright', 'book', 'checkmark', 'clipboard', 'close', 'connect', 'creditcard', 'dashboard', 'discord', 'github', 'home', 'link', 'mail', 'mail-fill', 'menu', 'person', 'plus', 'server', 'settings', 'shoppingcart', 'signout', 'tracker', 'undo'] as const;
export type Glyphs = typeof glyphs[number];
export interface GlyphProps { glyph: Glyphs; }
    ...
    case 'alerttriangle':
      return (
        <g>
          ...
          <circle cx="12" cy="16" r="1" /><path d="M12 8a1 1 0 0 0-1 1v4a1 1 0 0 0 2 0V9a1 1 0 0 0-1-1z" />
        </g>
      );
    case 'archive':
      return (
...
export const Icon: FC<IconProps> = ({ size = 24, onClick, glyph, fill }) => {
  ...
  return (
    ...
    >
      <InlineSvg
        ...
        viewBox={'0 0 24 24'} /* TODO: make responsive*/
